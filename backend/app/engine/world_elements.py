...
class PortalManager:
    ...
    def _spawn_portal(self, slot_idx):
        ...
        for _ in range(100):
            ...
            if not too_close and getattr(self.state, "bh_manager", None):
                for obh in self.state.bh_manager.black_hole_slots:
        ...
        if not p2:
            p2 = (
                (p1[0] + min_dist) % self.state.grid_width,
                (p1[1] + min_dist) % self.state.grid_height,
            )
    ...
    def update_slots(self, force_roll=False):
        ...
        if force_roll:
            self.portal_timers = [
                random.uniform(0.0, 60.0) for _ in range(target_count)
            ]
        ...
        if self.state.config.world.portals_enabled:
            for i in range(len(self.portal_slots)):
                ...
                if self.portal_timers[i] >= 60.0:
                    ...
                    should_exist = (
                        random.random() < self.state.config.world.portals_spawn_chance
                    )
                        ...
                        if (
                            ...
                            for pid, player in self.state.players.items():
                                if (
                                    ...
                                    and player.last_portal_exited is not None
                                ):
                                    if player.last_portal_exited[0] == p.id:
                                        in_use = True
                                        break

                            wait_time = getattr(p, "close_wait_time", 0.0)

                            if in_use and wait_time < 5.0:
                                # Wait another cycle or just delay timer slightly
                                p.close_wait_time = wait_time + 1.0
                                self.portal_timers[i] -= 1.0  # Try again in 1 second
                            else:
                                p.target_scale = 0.0
                                p.state = "collapsing"
                                p.close_wait_time = 0.0
            ...
            for i, p in enumerate(self.portal_slots):
                ...
                if p.target_scale == 1.0 and p.current_scale < 1.0:
                    p.current_scale = min(
                        1.0, p.current_scale + tick_interval / growth_time
                    )
                    if p.current_scale >= 1.0:
                        p.state = "active"
                elif p.target_scale == 0.0 and p.current_scale > 0.0:
                    p.current_scale = max(
                        0.0, p.current_scale - tick_interval / growth_time
                    )
    ...
    def check_teleport_start(self, new_head, player):
        if self.state.config.world.portals_enabled == 1 and self.portal_slots:
            for portal in self.portal_slots:
                if (
                    portal is None
                    or portal.state == "dead"
                    or portal.current_scale <= 0.01
                ):
                    continue
...
class BlackHoleManager:
    ...
    def update_slots(self, force_roll=False):
        ...
        if force_roll:
            ...
            for i in range(target_count):
                should_exist = (
                    random.random() < self.state.config.world.black_holes_spawn_chance
                )
                if should_exist:
        ...
        if self.state.config.world.black_holes_enabled:
            for i in range(len(self.black_hole_slots)):
                ...
                if self.black_hole_timers[i] >= 60.0:
                    ...
                    should_exist = (
                        random.random()
                        < self.state.config.world.black_holes_spawn_chance
                    )
            ...
            for i, bh in enumerate(self.black_hole_slots):
                ...
                if bh.target_scale == 1.0 and bh.current_scale < 1.0:
                    bh.current_scale = min(
                        1.0, bh.current_scale + tick_interval / growth_time
                    )
                    if bh.current_scale >= 1.0:
                        bh.state = "active"
                elif bh.target_scale == 0.0 and bh.current_scale > 0.0:
                    bh.current_scale = max(
                        0.0, bh.current_scale - tick_interval / growth_time
                    )
                    if bh.current_scale <= 0.0:
                        bh.state = "dead"
                        if (
                            hasattr(self.state, "food_manager")
                            and self.state.food_manager
                        ):
                            self.state.food_manager.drop_black_hole_food(bh)
                        self.black_hole_slots[i] = None
        else:
            for i in range(len(self.black_hole_slots)):
                self.black_hole_slots[i] = None
        ...
        if self.state.config.world.black_holes_enabled and self.black_hole_slots:
            for bh in self.black_hole_slots:
                ...
                if 0.001 < dist < eff_pull_radius:
                    ...
                    # Target angle directly towards the black hole
