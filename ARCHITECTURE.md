...
2. **Парсинг формул роста:**
   - **Файлы:** [FormulaParser.ts](file:///c:/Users/Engineer/Desktop/snake%20AI/frontend/src/worker/shared/FormulaParser.ts) и [game_config.py](file:///c:/Users/Engineer/Desktop/snake%20AI/backend/game_config.py)
   - **Что сломается:** Клиент и сервер рассчитают разную длину змейки при одном и том же счете (score). Это приведет к неверному построению сетки на клиенте, несовпадению толщины и некорректному расчету коллизий.
   - **Причина:** Сервер проверяет синтаксис формулы через AST-дерево Python, а клиентский воркер разбирает и вычисляет ее на JS.

3. **Сетевые константы физики:**
...
4. **Сериализация и декодирование сетевого кадра:**
