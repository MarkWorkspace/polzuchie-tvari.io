    ...
    return `
      <div class="admin-panel-layout" style="display: flex; flex-direction: column; height: 100vh; padding: 20px; gap: 20px; background: var(--bg-dark); box-sizing: border-box; overflow: hidden;">
        <header class="glass-panel" style="padding: 15px 20px; display: flex; justify-content: space-between; align-items: center; gap: 15px; flex-shrink: 0;">
          <div>
            <h2 style="margin: 0; font-size: 20px; font-weight: 800; background: linear-gradient(135deg, #fff, var(--accent)); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Admin Dashboard</h2>
      ...
      ${AdminDashboard.renderFloatingPanel(hasChanges, modifiedCount)}
      ${showRestartConfirm ? AdminDashboard.renderRestartModal() : ""}
    `;
  }
 
  private static renderHealth(h: any): string {
    if (!h.online) return `<span style="display: flex; align-items: center; gap: 5px;"><span class="pulse-dot" style="width: 8px; height: 8px; border-radius: 50%; background: #f87171; display: inline-block;"></span>Server: <strong style="color: #f87171; font-weight: 500;">offline</strong></span>`;
    return `
      <span style="display: flex; align-items: center; gap: 5px;">
        <span class="pulse-dot" style="width: 8px; height: 8px; border-radius: 50%; background: #4ade80; box-shadow: 0 0 6px #4ade80; display: inline-block;"></span>
        Server: <strong style="color: #4ade80; font-weight: 500;">online</strong>
      </span>
  ...
  private static renderTabBtn(tab: string, label: string, activeTab: string, count: number): string {
    ...
    return `
      <button class="tab-btn login-button" data-tab="${tab}" style="text-align: left; background: ${bg}; padding: 10px 14px; font-size: 13px; border-radius: 8px; margin: 0; box-shadow: none; display: flex; justify-content: space-between; align-items: center; width: 100%; box-sizing: border-box;">
        <span>${label}</span>
        ${count > 0 ? `<span style="background: ${active ? "#fff" : "#f59e0b"}; color: ${active ? "var(--accent)" : "#1e2025"}; padding: 1px 6px; border-radius: 10px; font-size: 11px; font-weight: 700; margin-left: 6px;">+${count}</span>` : ""}
      </button>
    `;
  }

  private static renderFloatingPanel(hasChanges: boolean, count: number): string {
    return `
      <div id="floating-action-panel" class="floating-action-panel" style="display: ${hasChanges ? "block" : "none"}; position: fixed; bottom: 30px; right: 30px; z-index: 1000; background: rgba(30, 32, 40, 0.98); border: 1px solid #f59e0b; border-radius: 12px; padding: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.5), 0 0 20px rgba(245,158,11,0.2); width: 280px; box-sizing: border-box;">
