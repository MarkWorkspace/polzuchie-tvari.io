...
interface LeaderboardProps {
  leaderboard: { id: string; score: number; kills: number; deaths: number }[];
  myId: string;
}

export const Leaderboard: React.FC<LeaderboardProps> = ({ leaderboard, myId }) => {
  return (
    <div style={{ backgroundColor: "#222", border: "2px solid #333", padding: "15px", borderRadius: "8px", color: "white", flex: "1 1 200px", minWidth: "200px" }}>
      ...
      <ul style={{ listStyle: "none", padding: 0, margin: 0, fontSize: "14px" }}>
        {leaderboard.map((player, index) => {
          ...
          const displayName = isMe ? "Вы" : player.id.split('_')[0];
          return (
            <li key={player.id} style={{ padding: "8px 0", color: isMe ? "#4ade80" : "white", fontWeight: isMe ? "bold" : "normal", borderBottom: "1px solid #444" }}>
